...
//    - P_/p_ to turn on air pressure (note also terminated with a "_").
//
...
int nanoled = 13;    // Nano board LED for testing
...
// Read the host serial commands
  ...
  while (Serial.available() > 0) {
    ...
    if (inchar == '_') {
      ...
      Serial.println(pulsetime);  // for testing
...
currentdispenseMillis = millis();
  if (currentdispenseMillis - previousdispenseMillis >= pulsetime) {
    ...
    digitalWrite(nanoled, LOW);       // For testing
    pulsetime = 0; }
  else {
    digitalWrite(dispensevalveoutput, HIGH);  // turn the valve on
    digitalWrite(nanoled, HIGH);      // For testing
    }
